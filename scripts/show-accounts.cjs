/**
 * Hardhat のデフォルトアカウント（localhost 用）を表示する。
 * 使い方: どのターミナルでも npm run accounts
 */
// Hardhat のデフォルトニーモニックから得られる固定アカウント（そのまま表示）
const ACCOUNTS = [
...
function main() {
  console.log("Hardhat ローカル用アカウント（先頭5件）:\n");
  for (const { index, address, privateKey } of ACCOUNTS) {
    console.log(`Account #${index}: ${address}`);
    console.log(`  Private Key: ${privateKey}`);
    console.log("");
  }
  console.log("MetaMask で「アカウントのインポート」→ 秘密鍵を貼り付けて Account #1 などを使う。");
}
