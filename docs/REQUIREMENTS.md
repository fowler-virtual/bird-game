...
| タブ | 内容・できること |
|------|------------------|
| **FARMING** | **Loft グリッド**（デッキに配置した鳥の一覧表示）。経過時間で **SEED** が増加（デッキに鳥が1羽以上いるとき）。**Save** ボタンでデッキをオンチェーンに保存し、SEED/DAY・Network Share を更新。**LOFT Upgrade** で $SEED を消費して枠を追加（2→4→6→8→10→12 枠）。 |
| **ADOPT** | **ガチャ**。$SEED を消費して鳥を1羽または10羽引く。獲得した鳥は待機リスト（インベントリ）に追加。初回プレイ時はここから誘導される。 |
| **LOFT**（Deck） | **デッキ枠（A〜L）** と **待機リスト（インベントリ）**。鳥をタップして配置先スロットを選ぶとデッキに配置。スロットをタップすると鳥を外せる。解放済み枠だけ配置可能（枠数は LOFT レベルで 2〜12）。 |
...
- **オンボーディング**: 初回は「ADOPT でガチャ → LOFT で鳥を配置 → FARMING で Save」の順に誘導。完了まで他タブはロックされる場合あり。
